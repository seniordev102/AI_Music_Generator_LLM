...
def lm_score(bars, note_string, bar_idx):
    tmp_string = []
...
if __name__ == '__main__':
    ...
    use_sentiment = config.sentiment
    global is_debug
    ...
    with open(lyrics_path) as lyrics_file, open(chord_path) as chord_file:
        lyrics_corpus = lyrics_file.readlines()
        chord_corpus = chord_file.readlines()
        for lyrics, chord in zip(lyrics_corpus, chord_corpus):
            ...
            save_demo(note_string, select_chords, name, lang,
